...
# Script to check for latest synq-dwh Docker image version and update deployment.yaml
...
# Image repository
IMAGE_REPO="europe-docker.pkg.dev/synq-cicd-public/synq-public/synq-dwh"
DEPLOYMENT_FILE="base/deployment.yaml"

echo "==> Checking for latest synq-dwh image version" >&2

# Get current version from deployment.yaml
CURRENT_VERSION=$(grep 'image: europe-docker.pkg.dev/synq-cicd-public/synq-public/synq-dwh:' "$DEPLOYMENT_FILE" | sed 's/.*synq-dwh://' | tr -d ' ')
echo "Current version: $CURRENT_VERSION" >&2

# Query Google Container Registry for available tags
# Using Docker Registry HTTP API v2 (public registry)
TAGS_JSON=$(curl -s "https://europe-docker.pkg.dev/v2/synq-cicd-public/synq-public/synq-dwh/tags/list")
...
sed -i.bak "s|image: europe-docker.pkg.dev/synq-cicd-public/synq-public/synq-dwh:.*|image: europe-docker.pkg.dev/synq-cicd-public/synq-public/synq-dwh:${LATEST_VERSION}|" "$DEPLOYMENT_FILE"
